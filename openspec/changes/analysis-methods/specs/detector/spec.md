...
The `package` field of each `FunctionRecord` SHALL be derived by a single shared `derive_package` helper (`PACKAGE_DERIVATION`): strip the `.py` extension from the file path relative to `root_path`, replace `/` with `.`, and drop a trailing `.__init__` (so `pkg/__init__.py` → `"pkg"`). This helper is shared by the detector and complexity modules only — coverage result rows carry `file` + `function` (no `package`) per the protocol schema, so coverage does NOT use `derive_package`. The stat / `S_ISREG` / byte-cap guard is centralized in the `is_analyzable_file` helper, which is shared by all three modules — detector and complexity reach it through the `iter_source_files` safe-file reader, while coverage calls it directly from its own per-file span reader — so no module re-implements the guard logic. The `iter_source_files` reader (which applies `is_analyzable_file` then `ast.parse` under the AST-depth budget and yields `(rel_path, source, tree)`) is used by the detector and complexity modules only; coverage does not use `iter_source_files`. The `enumerate_functions_with_spans` helper (per-function AST walk mapping line spans to function names) is used by coverage only, to map executed lines to the functions that contain them. The `ordered_file_list` helper is used by all three modules.
